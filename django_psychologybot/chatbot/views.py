...
def ask_openai_with_assistant(message, thread_id, report_id):
    ...
    if QUESTIONNAIRE_LABEL_RU in assistant_response or QUESTIONNAIRE_LABEL_KZ in assistant_response:
        ...
        try:
            ...
            word_file_path = os.path.join(settings.MEDIA_ROOT, 'reports', word_file_name)
            word_file_url = f"{settings.MEDIA_URL}reports/{word_file_name}"
            ...
            report.file = f"reports/{word_file_name}"  # Сохраняем относительный путь к файлу
            report.save()
